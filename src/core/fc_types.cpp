...
namespace fc
{
  ...
  {
    // First finish recording the command buffer
    vkEndCommandBuffer(wrapper.cmdBuffer);

    // Prepare the 2 optional semaphores that we can set to be waited on befor GPU processses cmdBuffer
    ...
    vkQueueSubmit2(mQueue, 1u, &submitInfo, wrapper.fence);
  ...
  bool VulkanImmediateCommands::isReady(const SubmitHandle handle) const
  {
    ...
    return vkWaitForFences(mDevice, 1, &cmdBuffer.fence, VK_TRUE, 0);
  }
