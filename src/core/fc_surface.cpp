...
namespace fc
{
  ...
  template void FcSurface::uploadMesh<Vertex>(std::span<Vertex> vertices, std::span<uint32_t> indices);
  // Instantiation of mesh utilized by loading height map
  template void FcSurface::uploadMesh<glm::vec3>(std::span<glm::vec3> vertices, std::span<uint32_t> indices);
  ...
  const bool FcSubmesh::isVisible(const glm::mat4& viewProjection) const
  {
    ...
    glm::mat4 mvp = viewProjection * parent.lock()->ModelMatrix();
  ...
  const bool FcSubmesh::isInBounds(const glm::vec4& position) const
  {
    ...
    for (size_t cornerIndex = 0; cornerIndex < 8; cornerIndex++)
    {
      ...
      glm::vec4 corner = parent.lock()->ModelMatrix() * boundaryBox[cornerIndex];
