...
namespace fc {
  ...
  // RenderObject and FcModel and FcMesh. For instance, you could still use a larger class
  // like this as long as everything is being done by pointers, etc. FcModel could contain
  // a RenderObject for instance NOTE: Model may no longer be viable since we are mostly
  // trying to create a scene graph where each node can have multiple children nodes but
  // only one parent node. This design allows us to have meshes that will get reused in
  // separate instances but within the same scene. Given that, FcModel should probably be
  // transformed into something like FcScene?
    class FcModel
    {
     private:
