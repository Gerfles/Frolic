...
// textures can contain a mix of 1D, 2D, 3D textures.
layout(set = 4, binding = 10) uniform sampler2D globalTextures[];
...
void main()
{
  ...
  // -*-*-*-*-*-*-*-*-*-*-*-*-*-   METALNESS / ROUGHNESS   -*-*-*-*-*-*-*-*-*-*-*-*-*- //
  float metalness = metalRoughFactors.x;
  float roughness = metalRoughFactors.y;

  if (metalRoughIndex != INVALID_TEXTURE_INDEX)
  {
    vec4 roughMetal = texture(globalTextures[nonuniformEXT(metalRoughIndex)], inUV);
  ...
  if (occlusionIndex != INVALID_TEXTURE_INDEX)
  {
    // TODO import occlussion factor
    ...
    occlusionFactor = 1.0 + occlusionFactor
                      * (texture(globalTextures[nonuniformEXT(occlusionIndex)], inUV).r - 1.0);
    // occlusionFactor = texture(globalTextures[nonuniformEXT(occlusionIndex)], inUV).r;
  }

  // *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-   EMISSION   *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*- //
  vec3 emissive = vec3(0.0);

  if (emissiveIndex != INVALID_TEXTURE_INDEX)
  {
    emissive = texture(globalTextures[nonuniformEXT(emissiveIndex)], inUV).rgb;
    emissive = decode_srgb(emissive) * vec3(emissiveFactors);
    // FIXME emissiveFactors.w is likely 1 but should either handle or eliminate comments
    // emissive = emissive * emissiveFactors.w;
  }
...
vec3 getMappedNormal()
{
  ...
  vec3 tangentNormal = normalize
                       (texture(globalTextures
                                [nonuniformEXT(normalIndex)], inUV).rgb * 2.0 -1.0);

  // *-*-*-*-*-*-*-*-*-   USING PASSED IN TANGENTS IF AVAILABLE   *-*-*-*-*-*-*-*-*- //
  if ((flags & HasVertexTangentAttribute) == HasVertexTangentAttribute)
  {
